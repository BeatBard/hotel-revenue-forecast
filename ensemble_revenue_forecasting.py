...
class HotelRevenueEnsemble:
    ...
    def engineer_features(self, splits: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        common_features = list(set(feature_cols) & set(val_features.columns) & set(test_features.columns))
    ...
    def remove_leakage_sources(self, dataset: Dict) -> Dict:
        ...
        # Remove variance threshold features
        var_threshold = VarianceThreshold(threshold=0.01)
        X_train_var = var_threshold.fit_transform(X_train)
        low_var_mask = var_threshold.get_support()
        low_var_features = [f for f, keep in zip(self.feature_names, low_var_mask) if not keep]
...
def main():
    ...
    results = ensemble.run_complete_pipeline(
        data_path='notebooks/revenue_center_data/RevenueCenter_1_data.csv',
